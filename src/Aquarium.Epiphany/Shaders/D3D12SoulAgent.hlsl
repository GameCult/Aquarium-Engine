...
SdfSurface sdfSurface(float3 p, int sdfIndex)
{
    ...
    float3 facetColor = lerp(float3(0.018, 0.024, 0.070), float3(0.16, 0.22, 0.56), confidence);
    ...
    surface.roughness = 0.040 * isFacet + 0.075 * isFinding + 0.12 * isRisk;
    surface.emission = findingColor * isFinding * (0.34 + confidence * 0.62)
        + riskColor * isRisk * (0.14 + risk * 0.46);
    return surface;
}
...
float3 soulCrystalShade(float3 p, float3 normal, int sdfIndex, SdfSurface surface)
{
    ...
    float3 pseudoRefraction = studioPmremSample(glassDirection, 2.2) * glassTint * 0.55;
    ...
    float internalChamber = smoothstep(0.72, 0.18, length(local));
    float pulse = 0.88 + 0.12 * sin(timeSeconds * 1.7 + confidence * 2.4);
    float3 oathLight = float3(1.0, 0.94, 0.72) * pulse * (0.44 + confidence * 1.10) * (centerGlow * 0.55 + internalChamber + rimGlow * 0.32);
    float3 edgeLight = float3(0.42, 0.70, 1.0) * (rimGlow * 0.72 + facetFlash * 0.42) * (0.7 + confidence * 0.7);
